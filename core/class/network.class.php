...
/*     * *********************DNS************************* */
	...
	public static function dns_create() {
		...
		if (!is_object($plugin)) {
			throw new Exception(__('Le plugin OpenVPN doit ̻tre install̩', __FILE__));
		}
		if (!$plugin->isActive()) {
		...
		if (!$plugin->isActive()) {
			throw new Exception(__('Le plugin OpenVPN doit ̻tre actif', __FILE__));
		}
	...
	public static function dns_start() {
		...
		if (!is_object($cmd)) {
			throw new Exception(__('La commande de d̩marrage du DNS est introuvable', __FILE__));
		}
		$cmd->execCmd();
	...
	public static function dns_stop() {
		...
		if (!is_object($cmd)) {
			throw new Exception(__('La commande d\'arr̻t du DNS est introuvable', __FILE__));
		}
		$cmd->execCmd();
	}
